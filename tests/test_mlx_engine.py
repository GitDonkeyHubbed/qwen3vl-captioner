...
def test_caption_video_stages_frames_and_matches_num_images(tmp_path, monkeypatch):
    ...
    # The clip framing counts the frames actually staged (3), not the 8 that
    # were requested — the same rule num_images follows below. It matches the
    # GGUF backend word for word via engine.base.frame_video_prompt.
    assert captured["messages"] == [
        {"role": "system", "content": DEFAULT_SYSTEM_PROMPT},
        {"role": "user", "content": frame_video_prompt("Describe the video.", 3)},
    ]
    assert "The 3 images above" in captured["messages"][1]["content"]
...
def test_caption_video_downscales_staged_frames(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        engine.video,
        "sample_frames",
        lambda path, n, cancel_check=None, max_dim=None: [
...
def test_caption_video_stages_frames_in_temporal_order(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        engine.video, "sample_frames", lambda path, n, cancel_check=None, max_dim=None: _solid_frames(4)
    )
...
def test_caption_video_keeps_frames_alive_through_streaming(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        engine.video, "sample_frames", lambda path, n, cancel_check=None, max_dim=None: _solid_frames(2)
    )
...
def test_caption_video_returns_empty_when_cancelled_during_extraction(
    ...
    monkeypatch.setattr(
        ...
        lambda path, n, cancel_check=None, max_dim=None: _solid_frames(4),
    )
    started = []
...
def test_caption_video_propagates_sampler_cancellation(tmp_path, monkeypatch):
    ...
    def cancelling_sampler(path, n, cancel_check=None, max_dim=None):
